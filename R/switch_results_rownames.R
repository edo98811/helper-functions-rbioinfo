...
#' If the `ensembl_gene_id` column is not present in the results table, it will be added using the `anno_df` annotation dataframe.
#'
#' @param results_table A data frame containing the results table. It should have a column named `uniprot_id`.
#' @param anno_df A data frame containing annotation information, which will be used to add gene information if necessary.
#'
...
#' \itemize{
#'   \item Checks if the `ensembl_gene_id` column is present in the results table. If not, it adds the gene information using `anno_df`.
...
#' anno_df <- data.frame(uniprot_id = c("P12345", "Q67890"), ensembl_gene_id = c("ENSG000001", "ENSG000002"))
#' results_to_gene_id(results_table, anno_df)
#' }
#'
#' @export
results_to_gene_id <- function(results_table, anno_df) {
    if (!"ensembl_gene_id" %in% colnames(results_table)) {
        results_table <- add_gene_info(results_table, anno_df)
    }
...
#' @param anno_df A data frame containing annotation information. It must have columns "unprot_id", "gene_symbol", and "ensembl_gene_id".
#'
...
#' anno_df <- data.frame(unprot_id = c("P12345", "Q67890", "A11111"), gene_symbol = c("GeneA", "GeneB", "GeneC"), ensembl_gene_id = c("ENSG000001", "ENSG000002", "ENSG000003"))
#' add_gene_info(results_table, anno_df)
#'
#' @export
...
#' @param anno_df A data frame containing the annotation information, where one of the columns is `gene_symbol` and the row names are protein IDs.
#'
#' @return A data frame with the same content as `results_table`, but with row names replaced by protein IDs from `anno_df`. The column `prot_ID` is removed from the final output.
#'
#' @examples
#' \dontrun{
#' results_table <- data.frame(matrix(ncol = 3, nrow = 3))
#' rownames(results_table) <- c("gene1", "gene2", "gene3")
#' anno_df <- data.frame(gene_symbol = c("gene1", "gene2", "gene3"), row.names = c("prot1", "prot2", "prot3"))
#' results_to_prot_id(results_table, anno_df)
#' }
#' @export
results_to_prot_id <- function(results_table, anno_df) {


    results_table$prot_ID <- rownames(anno_df[match(rownames(results_table), anno_df$gene_symbol), ]) # rownames are protID (magari da cambiare per interoperability)
