#' Run topGO Graph
#'
#' This function runs a topGO analysis to identify enriched GO terms in a set of differentially expressed genes.
#'
#' @param de_container A container object for differential expression analysis results. Default is NULL.
#' @param res_de A data frame containing the results of differential expression analysis. Default is NULL.
#' @param de_genes A vector of differentially expressed genes. Default is NULL.
#' @param bg_genes A vector of background genes. Default is NULL.
#' @param top_de A vector of top differentially expressed genes. Default is NULL.
#' @param FDR_threshold A numeric value specifying the false discovery rate threshold. Default is 0.05.
#' @param min_counts An integer specifying the minimum counts for filtering genes. Default is 0.
#' @param ontology A character string specifying the ontology to use ("BP" for Biological Process or "MF" for Molecular Function). Default is "BP".
#' @param annot A function for creating the topGO object. Default is annFUN.org.
#' @param mapping A character string specifying the annotation mapping to use. Default is "org.Mm.eg.db".
#' @param gene_id A character string specifying the type of gene identifier ("symbol" or "entrez"). Default is "symbol".
#' @param full_names_in_rows A logical value indicating whether to use full names in rows. Default is TRUE.
#' @param add_gene_to_terms A logical value indicating whether to add genes to terms. Default is TRUE.
#' @param de_type A character string specifying the type of differential expression ("up_and_down"). Default is "up_and_down".
#' @param topGO_method2 A character string specifying the second method for topGO analysis. Default is "elim".
#' @param do_padj A logical value indicating whether to adjust p-values. Default is FALSE.
#' @param verbose A logical value indicating whether to print detailed messages. Default is TRUE.
#'
#' @return A topGO result object containing the enriched GO terms.
#'
#' @examples
#' \dontrun{
#' result <- run_topGO_graph(de_container = my_de_container, res_de = my_res_de, de_genes = my_de_genes)
#' }
#'
#' @export
run_topGO_graph <- function(de_container = NULL,
                      res_de = NULL,
                      de_genes = NULL,
                      bg_genes = NULL,
                      top_de = NULL,
