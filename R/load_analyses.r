...
#'               \itemize{
#'                 \item \code{analysis_name} (optional): A character string specifying the name of the experiment. 
...
#'         }
#'
#' @details The function first checks if the \code{analysis_name} parameter is provided and if the corresponding 
#'          directory exists within the "analyses_data" folder. If so, it attempts to load the specified RDS files 
#'          from that directory. If any file is not found, a message is displayed. If \code{analysis_name} is not 
#'          provided or the directory does not exist, the function attempts to load the files from the current working 
#'          directory.
#'
#' @examples
#' \dontrun{
#' params <- list(analysis_name = "experiment1")
...
load_analyses <- function(params) {
    ...
    if (is.null(params$analysis_name)) {
        stop("The parameter 'analysis_name' is missing from the params and required.")
    }
    if ((is.character(params$analysis_name) & length(params$analysis_name)) && dir.exists(file.path("analyses_data", params$analysis_name))) {
        tryCatch({
            myresuSet <- readRDS(file.path("analyses_data", params$analysis_name, "myResuSet.RDS"))
            assign("myresuSet", myresuSet, envir = parent.frame())
        }, error = function(e) {
            message("File 'myResuSet.RDS' not found in experiment folder.")
        })
        tryCatch({
            dds <- readRDS(file.path("analyses_data", params$analysis_name, "dds_DE_results.RDS"))
            assign("dds", dds, envir = parent.frame())
        }, error = function(e) {
            message("File 'dds_DE_results.RDS' not found in experiment folder.")
        })
        tryCatch({
            dds_releveled <- readRDS(file.path("analyses_data", params$analysis_name, "dds_DE_results_releveled.RDS"))
        ...
        tryCatch({
            dds_interaction_term <- readRDS(file.path("analyses_data", params$analysis_name, "dds_DE_results_interaction_term.RDS"))
