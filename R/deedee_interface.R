...
#'
#' @export
dea_for_report_transcriptomics <- function(dea_table, anns, interactive = TRUE, alpha = 1, pvalue_column = "padj", species = "Mm") {
  if (alpha <= 1 && alpha >= 0) {
...
#'
#' @export
fea_for_report_transcriptomics <- function(fea_table, interactive = TRUE, alpha = 1, pvalue_column = "p.value_elim") {
  if (alpha <= 1 && alpha >= 0) {
    message("alpha threshold is valid.")
  } else {
    stop("alpha threshold must be between 0 and 1.")
